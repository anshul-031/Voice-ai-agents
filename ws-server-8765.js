...
wss.on('connection', (ws, req) => {
  ...
  const sampleRate = parseInt(url.searchParams.get('sample-rate') || SAMPLE_RATE);
      ...
      switch (event) {
        ...
        case 'media':
          ...
          if (data.media?.payload) {
            const audioChunk = base64ToUint8(data.media.payload);
            ...
            if (audioBuffer.length >= CHUNK_SIZE && !isProcessing) {
              ...
              try {
                ...
                const llmText = llmData.text || 'I apologize, I did not understand that.';
...
server.listen(WS_PORT, '0.0.0.0', () => {
  ...
  console.log(`   ws://34.143.154.188:${WS_PORT}?sample-rate=8000`);
