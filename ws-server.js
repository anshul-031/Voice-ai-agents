...
// Simple VAD controls (energy gating) to avoid sending silence/noise to STT
const VAD_ENABLED = (process.env.VAD_ENABLED || 'true').toLowerCase() !== 'false';
// If VAD_DBFS_THRESHOLD is set, it takes precedence (e.g., -45). Otherwise use RMS threshold.
const VAD_DBFS_THRESHOLD = Number.isFinite(parseFloat(process.env.VAD_DBFS_THRESHOLD || ''))
  ? parseFloat(process.env.VAD_DBFS_THRESHOLD)
  : null;
const VAD_RMS_THRESHOLD = Math.max(0, parseInt(process.env.VAD_RMS_THRESHOLD || '500', 10) || 500);
...
// Helper: Compute RMS and dBFS for PCM16LE mono buffer
function analyzePcm16Energy(pcmBuf) {
  try {
    if (!pcmBuf || pcmBuf.length < 4) return { rms: 0, dbfs: -Infinity };
    let sumSquares = 0;
    let count = 0;
...
wss.on('connection', (ws, req) => {
  ...
  async function ttsTextAndPlay(text, label = 'tts') {
    ...
    function muLawByteToLinear16(b) {
      ...
      // Clamp to int16 range to avoid RangeError in writeInt16LE
    ...
    function muLawToPCM16(buf) {
      ...
      for (let i = 0; i < buf.length; i++) {
        const s = muLawByteToLinear16(buf[i]);
        // s is already clamped but re-check defensively
        ...
        case 'media':
          ...
          if (data.media && typeof data.media.payload === 'string' && data.media.payload.length > 0) {
            ...
            if (audioBuffer.length >= CHUNK_SIZE && !isProcessing && !stopped && !closed) {
              ...
              // Process audio pipeline: VAD → STT → LLM → TTS
              try {
                // Lightweight VAD: compute energy; skip if below threshold
                if (VAD_ENABLED) {
                  const { rms, dbfs } = analyzePcm16Energy(processingBuffer);
                  // Decide using DBFS if configured, otherwise RMS
                  let voiced = true;
                  if (VAD_DBFS_THRESHOLD !== null) {
                    voiced = dbfs >= VAD_DBFS_THRESHOLD;
                  } else {
                    voiced = rms >= VAD_RMS_THRESHOLD;
                  }
                  if (!voiced) {
                    console.log(`[Exotel WS] VAD gate: below threshold (rms=${rms.toFixed(0)}, dbfs=${dbfs.toFixed(1)}), skipping STT`);
                    isProcessing = false;
                    return;
                  } else {
                    console.log(`[Exotel WS] VAD pass: rms=${rms.toFixed(0)}, dbfs=${dbfs.toFixed(1)}`);
                  }
                }
                ...
                if (!transcription.trim()) {
                  console.log(`[Exotel WS] STT returned empty text, skipping...`);
                  isProcessing = false;
                  return;
                }
