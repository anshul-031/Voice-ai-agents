...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // Attempt MongoDB connection for optional persistence
        let persist = false;
        try {
            await dbConnect();
            persist = true;
            console.log('[LLM] Connected to MongoDB (persistence enabled)');
        } catch (dbConnErr) {
            // Do NOT fail the request due to DB issues; continue without persistence
            persist = false;
            console.warn('[LLM] MongoDB not available, continuing without persistence:', (dbConnErr as any)?.message ?? dbConnErr);
        }
        ...
        if (persist) {
            try {
                ...
                console.error('[LLM] Failed to save user message (non-fatal):', dbError);
                // Continue with LLM request even if DB save fails
            }
        } else {
            console.log('[LLM] Skipping user message persistence (DB unavailable)');
        }
        ...
        try {
            ...
            if (persist) {
                try {
                    ...
                    console.error('[LLM] Failed to save assistant response (non-fatal):', dbError);
                    // Continue even if DB save fails
                }
            } else {
                console.log('[LLM] Skipping assistant response persistence (DB unavailable)');
            }
