    ...
    console.log('[upload-audio] Audio buffer size:', (audioBuffer as any).length, 'bytes');
        ...
        while (attempts < maxAttempts) {
            ...
            if (result.status === 'completed') {
                ...
                // Treat "no speech" / silence-like errors as non-fatal with empty text
                const isSilence = (
                    errMsg.includes('no speech') ||
                    errMsg.includes('no spoken audio') ||
                    errMsg.includes('silence') ||
                    errMsg.includes('could not detect any speech') ||
                    errMsg.includes('audio too short') ||
                    errMsg.includes('low sound') ||
                    errMsg.includes('insufficient audio') ||
                    errMsg.includes('invalid audio stream')
                );
                if (isSilence) {
                    console.warn('[upload-audio] Treating silence/no-speech as empty transcription (200)');
                    return NextResponse.json({ text: '', transcriptId: transcriptId });
                }
